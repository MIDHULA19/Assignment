...
import { Download, Award, Star, BookOpen, Presentation } from 'lucide-react';
...
const OttobonCertificate: React.FC<Props> = ({ data }) => {
  const getThemeVars = () => {
    switch (data.type) {
      case 'Excellence':
        return { headerBg: 'bg-brand-accent', textHi: 'text-brand-accent', icon: <Award size={32} className="text-brand-accent" /> };
      case 'Participation':
        return { headerBg: 'bg-green-600', textHi: 'text-green-600', icon: <BookOpen size={32} className="text-green-600" /> };
      case 'Project':
        return { headerBg: 'bg-purple-600', textHi: 'text-purple-600', icon: <Presentation size={32} className="text-purple-600" /> };
      case 'Completion':
      default:
        return { headerBg: 'bg-blue-600', textHi: 'text-blue-600', icon: null };
    }
  };
